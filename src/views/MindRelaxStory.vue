<template>
    ...
    <div class="container">
        ...
        <main>
            <section class="tabs">
                <button id="storyTab" class="tab-btn active">
                    舒缓故事
                </button>
            </section>
            ...
            <section class="player-section">
                ...
                <audio id="audioPlayer" ref="audioPlayer" controls @error="handleAudioError"
                    @loadeddata="handleAudioLoaded" @loadstart="handleAudioLoadStart"
                    @loadedmetadata="handleAudioMetadata" @canplay="handleAudioCanPlay" @waiting="handleAudioWaiting"
                    @playing="handleAudioPlaying" @stalled="handleAudioStalled" @abort="handleAudioAbort"
                    @suspend="handleAudioSuspend" @emptied="handleAudioEmptied"></audio>
            </section>
        </main>
    </div>
</template>
...
export default {
    ...
    setup() {
        ...
        // 计算属性方法 (仅保留故事相关)
        const getItemId = (item) => item.storyId;
        const getItemTitle = (item) => item.storyName;
        const getItemAuthor = (item) => item.author;
        ...
        const playAudio = async (id) => {
            ...
            // 获取音频流URL (故事) - 修复URL路径，保持与其他API一致
            const streamUrl = `${API_URLS.SOUL_ASSESS}/audio/story/${id}`;
        ...
        const viewBasicDetails = async (id) => {
            ...
            try {
                ...
                if (data.code === 200) {
                    ...
                    playerTitle.value = item.storyName;
                    playerAuthor.value = item.author;
                } else {
        ...
        const getAudioInfo = async (id) => {
            // 获取音频信息 (故事)
            const infoUrl = `/audio/info/story/${id}`;
